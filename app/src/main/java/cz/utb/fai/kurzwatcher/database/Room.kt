...
interface KurzDao {
    ...
    @Query("Select t.id, t.code, t.createdTime, t.rate, t.timestamp FROM DatabaseKurzEntry t inner join " +
            "(SELECT id, MIN(rate) as min_rate FROM DatabaseKurzEntry GROUP BY code) a " +
            "ON a.id = t.id and a.min_rate = t.rate")
    fun getLowestKurzes(): LiveData<List<DatabaseKurzEntry>>

    @Query("Select t.id, t.code, t.createdTime, t.rate, t.timestamp FROM DatabaseKurzEntry t inner join " +
            "(SELECT id, MAX(rate) as max_rate FROM DatabaseKurzEntry GROUP BY code) a " +
            "ON a.id = t.id and a.max_rate = t.rate")
    fun getHighestKurzes(): LiveData<List<DatabaseKurzEntry>>
...
@Database(entities = [DatabaseKurzEntry::class], version = 4)
